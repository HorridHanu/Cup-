...
def make_leet(x):
    """convert the string in leet"""
    for letter, letterleet in CONFIG["LEFT"].items():
        x = x.replace(letter, letterleet)
        return x
        # for concatenations...
...
def komb(seq, start, specail=""):
    for mystr in seq:
        for mystr1 in start:
            yield mystr + specail + mystr1

            # print list to file counting file
...
def print_to_file (filename, unique_list_finished):
    ...
    print(
        "[+] Saving dictiornary to\033[1; 31m "
        + filename
    ...
    print(
        "[+] Now you Load your password with\033[1; 31m"
        +filename
        +"\033[1; m and Now Shoot! Good Luck"
...
def print_Hanu():
    print("\r@@\t\t@\t@@@@@@@@@\t@@@     @\t@@\t\t@\n"
          ...
          "@@\t\t@\t@@\t\t@\t@@     @@\t@@\t\t@\n"
          "@@\t\t@\t@@\t\t@\t@@\t\t@\t@@@@@@@@@\r")
    print(8 * " " + "[+] [ HorridHanu | https://github.com/HorridHanu/]\r\n")



def version():
    """Display version"""
    print("\r\n	\033[1;31m[ Main.py ]  " + __version__ + "\033[1;m\r\n")
...
def improve_dictionary(file_to_open):
    ...
    if not os.path.isfile(file_to_open):
        exit("[-] ERROR : file" + file_to_open + "does not exits.")
    ...
    Tanach = open(file_to_open, "r")
    listic = Tanach.readlines()
    listica = []
    for x in listic:
        listica += x.split()
    ...
    if conts =="y" and len(listic)>CONFIG["global"]["threshold"]:
        ...
        print("[-] check the configuration file with the increasing this number\r\n")
        conts = input(
            "> Do you want to concatenate all words from wordlist? Y/[N]: "
        ).lower()
    ...
    if spechar1 == "y":
        for spec1 in chars:
            spechar.append(spec1)
            for spec2 in chars:
                spechar.append(spec1+spec2)
                for spec3 in chars:
                    spechar.append(spec1+spec2+spec3)
    ...
    if spechar1 == "y":
        kombinacija[2] = list(komb(listica, spechar))
        if conts == "y":
            kombinacija[3] = list(komb(cont, spechar))
    if randnum =="y":
        kombinacija[4] = list(komb(listica, numfrom, numto))
        if conts =="y":
            kombinacija[4] = list(komb(cont, numfrom, numto))
    ...
    for i in range(6):
        kombunique[i] = list(dict.fromkeys(kombinacija[i]).keys())

    kombunique[6] = list(dict.fromkeys(listica).keys())
    kombunique[7] = list(dict.fromkeys(cont).keys())


    """join The list"""
    uniqlist = []
    for i in range(8):
        uniqlist += kombunique[i]

    unique_hanu = list(dict.fromkeys(uniqlist).keys())
    uniqee_leet = []
    ...
    for (
        ...
        x = make_leet(x)
        uniqee_leet.append(x)

    uniqlist = unique_hanu + uniqee_leet
    ...
    print_to_file(file_to_open + ".cup.txt", uniqlist_finised)
...
def interactive():
    """Implementation of the -q switch. Interactively question the user and
    ...
    while len(name) == 0 or name == "" or name == " " or name == "   ":
        print("[-] You must at least enter name!")
    ...
    """Nickname"""
    profile["nick"] = input("> Nickname: ")

    """Birthdaydate"""
    birthdate = input("> Birthdate (DDMMYYYY): ")
    while len(birthdate) != 0 and len(birthdate) != 8:
        print("[-] You must enter 8 digit for birhtdate!")
        birthdate = input("> Birthdate (DDMMYYYY): ")
    ...
    """Birthdaydate"""
    wifeb = input("> Birthdate (DDMMYYYY): ")
    while len(wifeb) != 0 and len(wifeb) != 8:
        print("[-] You must enter 8 digit for birthdate!")
        wifeb = input("> Birthdate (DDMMYYYY): ")
    ...
    while len(kidb) != 0 and len(kidb) != 8:
        print("\r\n[-] You must enter 8 digits for birthdate!")
        kidb = input("> Child's birthdate (DDMMYYYY): ")
    ...
    word1 = input(
        ">[+] Do you want to add some keywords about victim? Y/[N]: "
    ).lower()
    word2 = ""
    if word1 == "y":
        word2 = input("> Please enter the words, separated by comma. [i.e. horrid,hacker,hanu], spaces will be removed: "
                      ).replace(" ", "")

    profile["words"] = word2.split(",")


    profile["spechar1"] = input(
        "> Do you want to add special chars at the end of words? Y/[N]: "
    ).lower()

    profile["randnum"] = input(
        "> Do you want to add random number at the end of words? Y/[N]: "
    ).lower()

    profile["leetmode"] = input(
        "> Leet mode? (i.e. 1337) Y/[N]: "
    ).lower()


    generate_word_list_from_profile(profile)



def generate_word_list_from_profile(profile):
    """generating the word list form given profile ;)"""
    chars = CONFIG["global"]["chars"]
    years = CONFIG["global"]["years"]
    numto = CONFIG["global"]["numto"]
    numform = CONFIG["global"]["numfrom"]

    profile["spechar"] = []
    for spec1 in chars:
        profile["spechar"].append(spec1)
        for spec2 in chars:
            profile["spechar"].append(spec1 + spec2)
            for spec3 in chars:
                profile["spechar"].append(spec1 + spec2 + spec3)

    print("\r\n[+] Now making a dictinary...")
    ...
    birthdate_dd = profile["birthdate"][:1]
    ...
    kidn = profile["kidn"].title()
    petup = profile["pet"].title()
    companyup = profile["company"].title()

    wordsub = list(map(str.title, profile["words"]))

    word = profile["words"] + wordsub

    # reverse a name

    rev_name = profile["name"][::-1]
    rev_nameup = nameup[::-1]
    rev_nick = profile["kid"][::-1]
    rev_nickup = nickup[::-1]
    ...
    if profile["randnum"] == "y":
        kombi[12] = list(concats(word, numform, numto))
        kombi[13] = list(concats(kombinaa, numform, numto))
        kombi[14] = list(concats(kombinaac, numform, numto))
        kombi[15] = list(concats(kombinaaw, numform, numto))
        kombi[16] = list(concats(kombinaak, numform, numto))
        kombi[21] = list(concats(reverse, numform, numto))
    ...
    for i in range(1, 22):
        ...
        komb_unique[i] = list(dict.fromkeys(kombina).keys())
    ...
    if profile["leetmode"] == "y":
        for (
                ...
                unique_lista
        ):  # if you want to add more leet chars, you will need to add more lines in cup.cfg too...
...
def download_https(url, targetfile):
...
def alectodb_download():
    ...
    user_ = list(set(username))
    pass_ = list(set(password))
    ...
    f.write(os.linesep.join(user_))
    f.close()

    f = open("alectodb-usernames.txt", "w")
    f.write(os.linesep.join(pass_))
    f.close()
...
def download_wordlist_http(filedown):
    ...
    make_dir_if_not_exist("dictionaries")
    ...
    if filedown in arguments:
        dire = "dictanories" + arguments[filedown][0] + "/"
        make_dir_if_not_exist(dire)
        file_to_down = arguments[filedown][1]

        for fi in file_to_down:
            url = CONFIG["global"]["dicturl"] + arguments[intfiledown][0] + "/" + fi
            tgt = dire + fi
            download_https(url, tgt)
        print("[+] files saved to " + dire)



def make_dir_if_not_exist(dire):
...
def get_parser():
    ...
    group.add_argument(
        ...
        help="Parse default usernames and passwords directly"
             " from HorridHanu. Project Alecto uses purified"
